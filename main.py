...
from utils.http import safe_request, yf_chart
...
def check_password():
    """顯示密碼輸入框；使用 hmac.compare_digest 防 timing attack，連續失敗 5 次鎖定 5 分鐘"""
    ...
    if pwd:
        ...
        # hmac.compare_digest 防止 timing attack；兩邊必須相同 type
...
def _compute_market_compass():
    ...
    try:
        ...
        _dh.record("台灣期交所", ok=not df_inst.empty)
    ...
    try:
        ...
        _dh.record("台灣期交所", ok=not df_pc.empty)
        if len(df_pc) >= 2:
...
st.sidebar.divider()
page = st.sidebar.radio("選擇頁面", ["🏠 選股系統", "🌍 總經儀表板", "📰 新聞監控", "📊 散戶指標", "📈 個股監控", "🌡️ 板塊熱力圖", "🔬 產業研究", "🎙️ Podcast 整理", "🤖 AI 問答"], key="nav_page")
_dh.render_sidebar()
...
# ==================== 總經儀表板 ====================
    ...
    with st.spinner("載入趨勢圖..."):
        try:
            ...
            if df_jul.empty and df_sep.empty and df_nov.empty:
                st.warning("Fed 利率趨勢圖暫時無法取得，請稍後重試")
            else:
                fig1 = go.Figure()
                if not df_jul.empty:
    ...
    with st.spinner("載入日銀趨勢圖..."):
        try:
            ...
            if df_boj.empty:
                st.warning("日銀政策趨勢圖暫時無法取得，請稍後重試")
            else:
                fig2 = go.Figure()
...
# ==================== AI 問答 ====================
    ...
    def _build_qa_context():
        parts = []
        pod_raw = gsheets.load("podcasts", POD_FILE_QA, {})
        if isinstance(pod_raw, list):
            pod_raw = {"__channels__": [], "episodes": pod_raw}
        episodes = pod_raw.get("episodes", [])
        ...
        names = research.get("__names__", {})
        parts.append("\n=== 個股研究筆記 ===")
        for ticker, notes in research.items():
            if ticker.startswith("__"):
                continue
            for n in notes:
                block = (f"[{n.get('date','')}] {ticker} {names.get(ticker,'')} - "
                         f"{n.get('title','')}\n{n.get('content','')}\n")
                parts.append(block)

        industry = gsheets.load("industry_research", INDUSTRY_FILE_QA, {})
        parts.append("\n=== 產業研究筆記 ===")
        for ind_name, notes in industry.items():
            for n in notes:
